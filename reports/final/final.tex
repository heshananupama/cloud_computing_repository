...
Virtual machines are much suitable for tasks that require high capacity disk locality such as relational databases. If there is not many traffic and if the requirement for scaling is lower, then VM's would be a better option as it's much easy to get start with and do not require resources that would typically needed in container environment. Other than that, if you need to build a legacy system which manage multiple applications on a single server and requires most resources and functionalities of an OS then it would be better to go with VM's. Though it was claimed that VM's ensure full isolation and security than containers, with the use of technologies like Firecracker had made the containers equally isolated and secured as VM's.

In the case of containers, they are much useful and easier to manage for stateless services. Containers are also a perfect fit for Micro-service architectures where the an application structure as a collection of services that are loosely coupled, highly maintainable and testable. Presence of state of art orchestration systems like kubernetes will also increase the manageability of these architectures. Finally, containers would also suitable for technologies such as lambda which executes your program logic once required and have the capability to handle burst workloads, from a few requests to thousands per second without having to pay much.
